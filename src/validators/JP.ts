...
function weightedSum(number: string, config: { modulus?: number; reverse?: boolean; weights: number[] }): number {
  const { modulus, reverse, weights } = config;
  let sum = 0;
  const digits = number.split('').map((num: string) => parseInt(num, 10));
...
function validate_jp_cn(input: string, debug: boolean = false): boolean {
