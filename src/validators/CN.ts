...
function validate_cn_uscc(uscc: string, debug: boolean = false): boolean {
    ...
    function weightedSum(number: string, config: { weights?: number[]; modulus?: number; alphabet?: string }) {
        var weights = config.weights || [1];
        var modulus = config.modulus || 10;
        var alphabet = config.alphabet || '0123456789';

        var digits = number.split('').map(function (num: string) {
