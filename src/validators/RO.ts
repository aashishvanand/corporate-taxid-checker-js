...
function isValidDate(yy: string, mm: string, dd: string): boolean {
  ...
  return date.getFullYear() == Number(yy) && date.getMonth() + 1 == Number(mm) && date.getDate() == Number(dd);
}
...
function validate_ro_cnp(input: string, debug: boolean = false): boolean {
  ...
  const year = (century as Record<string, string>)[centuryCode] + value.substr(1, 2);
...
function validate_ro_cui(input: string, debug: boolean = false): boolean {
  ...
  const checksum = value[value.length - 1];
  const rawSum = weightedSum(value.substring(0, value.length - 1), weights) * 10;
  const calculatedCheck = rawSum % 11 === 10 ? 0 : rawSum % 11;

  if (String(calculatedCheck) !== checksum) {
    if (debug) { console.log("Invalid checksum"); }
      return false;
  }
...
function validate_ro_cif(input: string, debug: boolean = false): boolean {
    if (input.length === 13) {
