function validate_cl_rut(rut: string, debug: boolean = false): boolean {
    ...
    function weightedSum(number: string, config: { reverse?: boolean; weights?: number[]; modulus?: number }) {
        var reverse = config.reverse || false;
        var weights = config.weights || [1];
        var modulus = config.modulus || 10;

        var digits: number[] = number.split('').map(function (num: string) {
