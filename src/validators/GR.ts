...
function validate_gr_vat(input: string, debug: boolean = false): boolean {
  // Strip EL prefix (EU VAT prefix for Greece)
  const value = input.toUpperCase().startsWith('EL') ? input.substring(2) : input;
